...
/**
 * MobileClubsController
 *
 */
class MobileClubsController extends MobileAppController {
	...
	public function index($q = "", $l = ""){
		...
		try{
			...
			$clubs = $this->requestAction("/clubs/index?q=$q&l=$l");
			//$this->log("clubs=" . var_export($clubs, true), 'debug');
			$items = array();
			
			foreach($clubs as $club){
				$item = array();
				$item["model"] = "club";
				$item["id"] = $club['Club']['id'];
				$item["name"] = $club["Club"]["clb_name"] ;
				$item["icon"] = "/images/image?id=" .  $club["Club"]["clb_logo_img_id"] . "&width=32&height=32";
				$item["value"] =  "<strong>" . __("Value") . ":</strong>"  . "USD$" . $club['Club']['clb_value'];
				$item["ranking"] = "<strong>" . __("Ranking") . ":</strong>"  . '#' .   $club['Club']['clb_ranking'];
				$item["uniqueid"] = $club['Club']['clb_ranking'];
				$items[] = $item;
			}
	...
	public function view($id = null){
		$this->viewClass = "Json";
		try{
			$club = $this->requestAction("/clubs/view/$id");
			$items = array();
			$item = array();
			$item["model"] = "club";
			$item["id"] = $id;
	...
	public function info($id = null){
		...
		try{
			...
			$attributes = $this->requestAction("/attributes/table/Club/$id");
	...
	public function comments($id = null, $max_id = 0, $date = 0){
		...
		try{
			...
			$comments = $this->requestAction("/clubs/comments/$id/0/${max_id}/$date");
			...
			foreach($comments as $comment){
				...
				$child['id'] = 'com' . $com_id ;
				$child['raw_id'] = $com_id ;
				$child['club_id'] = $comment['ComPro']['pro_club_id'];
	...
	public function stats($id = null){
		...
		try{
			...
			$child['model'] = 'stat';
			$child['id'] = "sta1";
			$child['label'] =  __("Goals per Player");
			$series = array(
				array(
					"caption" => __("Attempt"),
					"column" => "attempts"
			...
			$data = $this->requestAction("/clubs/goals_per_player/$id");
			$child['data']= json_encode($data);
			$children[] = $child;
			
			$child['model'] = 'stat';
			$child['id'] = 'sta2';
			$child['label'] = __("Cards Per Player");
			$series = array(
				array(
					"caption" => __("Yellows"),
					"column" => "yellows"
			...
			$child['series'] = json_encode($series);
			$data = $this->requestAction("/clubs/cards_per_player/$id");
			$child['data']= json_encode($data);
			$children[] = $child;
			
			$child['model'] = 'stat';
			$child['id'] = 'sta3';
			$child['label'] = __("Points per Year");
			$series = array(
				array(
					"caption" => __("Points"),
					"column" => "points"
			...
			$child['series'] = json_encode($series);
			$data = $this->requestAction("/clubs/points_per_year/$id");
			$child['data']= json_encode($data);
			$children[] = $child;
	...
	public function news($id = null, $since_id = 0, $max_id = 0){
		...
		try{
			...
			$account = $club['Club']['clb_twitter_account'];
