...
class ExtendedFormHelper extends FormHelper
{
    ...
    public function input($fieldName, $options = array()){
    	...
    	if ($type === 'file'){
    		//	-handle type=file, show a  SWFUpload file input fied  with name (data[Model][fieldName])
    	...
    	if ($type === 'video'){
    		//	-handle type=video, show a  SWFUpload file input fied  with name (data[Model][fieldName])
    		return $this->video($fieldName, $options);
    	}
    ...
    public function video($fieldName, $options = array()){
    	$model = $this->model();
    	$fieldId = $model . Inflector::camelize($fieldName);
    	$fieldValue = isset($options['value'])? $options['value'] : $this->value($fieldName, $options);
    	$fieldLabel = Inflector::humanize($fieldName);
    	...
    	if(isset($options['label']))
    		$fieldLabel = $options['label'];
    	$result = "<div class=\"input video\">\n" . 
    		       		"<label for=\"{$fieldId}\"><strong>{$fieldLabel}</strong></label>\n" .
    		      	       "<input type=\"hidden\" name=\"data[{$model}][{$fieldName}]\"  id=\"{$fieldId}\" value=\"{$fieldValue}\">" .
    		      	"</div>";
    	$result .= "<script type=\"text/javascript\">
				$(function(){
					$('#{$fieldId} ').videoUploader({
						width: 213,
						height: 236,
						editable: {$fieldEditable},
						default_url: '{$defaultUrl}'
    ...
    public function data($fieldName, $options = array()){
    	$this->log("<data fieldName=$fieldName>", 'debug');
    	$model = $this->model();
    	if(strstr($fieldName, ".")){
    		$this->log("field name has . in it", 'debug');
    		list(, $fieldName) = explode(".", $fieldName);
    		$this->log("new field Name is $fieldName", 'debug');
    	}
	...
	//if($field && $field !== 'id')
	//	$use_codes = 1;
	$url = "/$controller/autocomplete?use_codes=${use_codes}&value=${fieldValue}";
	$dataMap = $this->requestAction($url);
	$fieldDisplayValue = isset($dataMap[$fieldValue])? $dataMap[$fieldValue]: $fieldValue;
	
	//if fieldId has '.' replace it with '_'
							...
							if(target){
								...
								if(\$target.val() != suggestion.data){
									...
									$.each(dependents, function(i,dependent){
										if(dependent){
											$('input[id$=' + dependent + '_display]').val('');
											$('input[id$=' + dependent + ']').val('');
										}
									});
								}
							}
	...
	//add label
	$result = "<div class=\"input textarea\">\n" .
			"<label for=\"{$fieldId}\">{$fieldLabel}</label>\n";
