define("dojo/back", ["./_base/config", "./_base/lang", "./sniff", "./dom", "./dom-construct", "./_base/window", "require"],
	...
	function loadIframeHistory(){
		...
		if(historyIframe){
			...
			//0 && console.warn("dojo/back: Not initialised. You need to call back.init() from a <script> block that lives inside the <body> tag.");
		}
		return url; //String
	}
	...
	back.addToHistory = function(args){
		...
		if(!historyIframe){
			if(config["useXDomain"] && !config["dojoIframeHistoryUrl"]){
				0 && console.warn("dojo/back: When using cross-domain Dojo builds,"
