	...
	return declare([_WidgetBase],{
		...
		postCreate: function(){
			...
			on(this.domNode, 'busy', function(e){
				0 && console.debug("busy" + e.state);
				self.busy = e.state;
				var overlay = dijitRegistry.byId(this.id + "_overlay");
				if(self.busy && overlay){
					overlay.show();
					0 && console.debug("showing overlay...");
				}else
				if(overlay){
					overlay.hide();
					0 && console.debug("hiding overlay...");
				}
			});
	...
	return declare([MobileScrollableView,  _AppViewMixin, _TemplatedMixin, _WidgetsInTemplateMixin],{
		...
		postCreate: function(){
			...
			on(this.enrollButton.domNode, 'click', function(e){
				0 && console.debug("enroll clicked");
		...
		__doEnroll: function(transactionCode){
			var self = this;
			0 && console.debug("creating new student enrollment");
			dojoRequest.post(this.serviceUrl, {
				...
				0 && console.debug("enroll request returned");
				if(!response.studentid || !response.courseid ){
					0 && console.debug("there was an error during enrollment:" + response.error);
					Sys.message(response.error);
				}
				else{
					0 && console.debug("enrollment was successfull");
...
define([
	...
	return declare([MobileScrollableView,  _AppViewMixin, _TemplatedMixin, _WidgetsInTemplateMixin],{
		...
		__doLogin: function(telephone, password){
			var self = this;
			0 && console.debug("authenticating user against url:" + this.serviceUrl);
			dojoRequest.post(this.serviceUrl, {
				data: {
				...
				0 && console.debug("auth request returned");
				if(!response.userid){
					0 && console.debug("there was an error during login:" + response.toString());
					Sys.message(response.error);
				}
				else{
					0 && console.debug("login was successfull");
...
define([
	...
	return declare([MobileScrollableView, _TemplatedMixin, _WidgetsInTemplateMixin, _AppViewMixin],{
		...
		updateData: function(){
			var self = this;
			0 && console.debug("updatign settings data against url:" + this.serviceUrl);
			dojoRequest.post(this.serviceUrl, {
				data: {
				...
				if(!response.personid){
					0 && console.debug("there was an error during update:" + response.toString());
					Sys.message(response.error);
				}
				else{
					0 && console.debug("update was successfull");
				}
			}, function(error){
				self.alert(self.errors.err101);
			});
