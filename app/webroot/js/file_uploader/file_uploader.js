...
(function($) {
    $.fn.fileUploader = function(options) {
        return this.each(function() {
            ...
            var defaults = {
            	name: 'data[Document][file]',
            	target: id,
            	default_url: '/js/file_uploader/document.png',
            	value: val,
            	url_pattern: '/documents/icon?id={0}',
            	editable: true,
            	...
            	flash_url: '/js/file_uploader/swfupload.swf',
            ...
            var container = $("<div class='file-uploader'/>");
            container.append($("<div id='file-uploader-progress-bar'> <div>&nbsp;</div> </div>"));
            container.append($("<div id='file-uploader-message'/>"));
            container.append($("<div  id='file-uploader-cancel'>"  + uploading_caption +
            ...
            container.append($("<div id='file-uploader-preview'><img src='"+ img_src +"' width='"+ settings.width +"' height='"+ settings.height +"'></div>"));
         
            if(settings.editable){
            	container.append($("<div id='file-uploader-swf'/>"));
            	container.append($("<div id='file-uploader-button'><button style='" + settings.button_style + "' >" + upload_caption +"</button></div>"));
            }
           
            $(this).before(container);
            $("#file-uploader-progress-bar", container).hide();
            $("div[id=file-uploader-cancel]", container).hide();
             if(!settings.editable)
            	return;
		...
		file_queue_limit : 1,
                ...
                button_placeholder_id: 'file-uploader-swf',
                button_text: null,
		...
		button_window_mode : SWFUpload.WINDOW_MODE.TRANSPARENT,
                ...
                upload_start_handler: function() {
                    ...
                    $("#file-uploader-progress-bar  div", container).css("width", "0px");
                    $("#file-uploader-progress-bar", container).show();
                    $("div[id=file-uploader-cancel]", container).show();
                    $("div[id=file-uploader-cancel]", container).html("").hide();
                    $('#file-uploader-message', container).html("").hide();
                    $('div[id=file-uploader-button] button', container).hide();
                },
		    ...
		    if(json && json.file_id){
		    	...
		    	if(json.message)
                    ...
                    //$("div[id=file-uploader-message]", container).html(message);
                    if(file_id){
                    	var file_url = settings.url_pattern
                    			.replace("{0}", file_id)
                    			.replace("{1}", settings.width)
                    			.replace("{2}", settings.height);
                    	$("div[id=file-uploader-preview] img", container).attr('src', file_url);
                    	
                    	$("#" + settings.target).val(file_id);
                    }
                },
                ...
                upload_complete_handler: function() {
                    var clearup = function() {
                        $("div[id=file-uploader-progress-bar]", container).hide();
                        $("div[id=file-uploader-message]", container).show();
                        $("div[id=file-uploader-cancel]", container).hide();
                        swfu.setButtonDimensions(settings.width, settings.buttonHeight );
                        $('div[id=file-uploader-button] button', container).show();
                    };
                    clearup();
                },
                ...
                upload_progress_handler: function(file, bytes, total) {
                    ...
                    $("div[id=file-uploader-progress-bar] div", container).animate({ width: percent + "%" }, { duration: 500, queue: false });
                }
            };
            swfu = new SWFUpload($.extend(swfu_defaults, settings || {}));
	    $button = $('div[id=file-uploader-button] button', container);
	    swfu.button = $button;
	    
            // Called when user clicks "cancel" (forces the upload to end, and eliminates progress bar immediately)
            $("div[id=file-uploader-cancel] input[type='button']", container).click(function() {
