    ...
    $.fn.imageUploader = function(options) {
        return this.each(function() {
            ...
            var defaults = {
            	...
            	target: id,
            	default_url: '/js/image_uploader/anonymous.jpg',
            	value: val,
            	url_pattern: '/images/image?id={0}&width={1}&height={2}',
            	editable: true,
            	...
            	flash_url: '/js/image_uploader/swfupload.swf',
            ...
            container.append($("<div id='image-uploader-progress-bar'> <div>&nbsp;</div> </div>"));
            container.append($("<div id='image-uploader-message'/>"));
            container.append($("<div  id='image-uploader-cancel'>"  + uploading_caption +
            ...
            container.append($("<div id='image-uploader-preview'><img src='"+ img_src +"' width='"+ settings.width +"' height='"+ settings.height +"'></div>"));
         
            if(settings.editable){
            	container.append($("<div id='image-uploader-swf'/>"));
            	container.append($("<div id='image-uploader-button'><button style='" + settings.button_style + "' >" + upload_caption +"</button></div>"));
            }
           
            $(this).before(container);
            $("#image-uploader-progress-bar", container).hide();
            $("div[id=image-uploader-cancel]", container).hide();
             if(!settings.editable)
            	return;
		...
		file_queue_limit : 1,
                ...
                button_placeholder_id: 'image-uploader-swf',
                button_text: null,
		...
		button_window_mode : SWFUpload.WINDOW_MODE.TRANSPARENT,
                ...
                upload_start_handler: function() {
                    ...
                    $("#image-uploader-progress-bar  div", container).css("width", "0px");
                    $("#image-uploader-progress-bar", container).show();
                    $("div[id=image-uploader-cancel]", container).show();
                    $("div[id=image-uploader-cancel]", container).html("").hide();
                    $('#image-uploader-message', container).html("").hide();
                    $('div[id=image-uploader-button] button', container).hide();
                },
		    ...
		    var image_id = 0;
		    var message = '';
		    if(json && json.image_id){
		    	image_id = json.image_id;
		    	if(json.message)
		    		message = json.message;
		    }
                    //$("div[id=image-uploader-message]", container).html(message);
                    if(image_id){
                    	var image_url = settings.url_pattern
                    			.replace("{0}", image_id)
                    			.replace("{1}", settings.width)
                    			.replace("{2}", settings.height);
                    	$("div[id=image-uploader-preview] img", container).attr('src', image_url);
                    	
                    	$("#" + settings.target).val(image_id);
                    }
                },
                ...
                upload_complete_handler: function() {
                    var clearup = function() {
                        $("div[id=image-uploader-progress-bar]", container).hide();
                        $("div[id=image-uploader-message]", container).show();
                        $("div[id=image-uploader-cancel]", container).hide();
                        swfu.setButtonDimensions(settings.width, settings.buttonHeight );
                        $('div[id=image-uploader-button] button', container).show();
                    };
                    clearup();
                },
                ...
                upload_progress_handler: function(file, bytes, total) {
                    ...
                    $("div[id=image-uploader-progress-bar] div", container).animate({ width: percent + "%" }, { duration: 500, queue: false });
                }
            };
            swfu = new SWFUpload($.extend(swfu_defaults, settings || {}));
	    $button = $('div[id=image-uploader-button] button', container);
	    swfu.button = $button;
	    
            // Called when user clicks "cancel" (forces the upload to end, and eliminates progress bar immediately)
            $("div[id=image-uploader-cancel] input[type='button']", container).click(function() {
