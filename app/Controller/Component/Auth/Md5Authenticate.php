 ...
 * }}}
 *
 * When configuring AdminAuthenticate you can pass in settings to which fields, model and additional conditions
 * are used. See FormAuthenticate::$settings for more information.
 *
 * @see AuthComponent::$authenticate
 */
class Md5Authenticate extends FormAuthenticate {
 ...
 */
	public function authenticate(CakeRequest $request, CakeResponse $response) {
		...
		$userModelObj->log('<Md5.authenticate>', 'debug');
		...
		try{
			if(!$request->isPost()){
				$userModelObj->log("request is not post, returning true", 'debug');
				return false;
			}
			...
			$telephone = $request->data($model . '.' . $fields['username'] ) ;
			$passwd = $request->data($model . '.' . $fields['password']);
			
			$userModelObj->log("telephone=$telephone,passwd=$passwd", 'debug');
		
			//check of a users exists with the specified telephone
			//NOTE: telephones are case insensitive
			$fieldName = $fields['username'];
			$userModelObj->log("fieldName=${fieldName}", 'debug');
			$conditions = array("LOWER(User.{$fieldName})"=> strtolower($telephone));
			
			$result = ClassRegistry::init($userModel)->find('first', array(
						'conditions' => $conditions));
			if(!$result){
				$error = __('Unknown user');
				throw new Exception($error);
			}
			//a user exists
			...
			foreach($dependents as $dependentAlias => $dependent){
				...
				if($dependentObj['id']){
					$user = array_merge($dependentObj, $user);
				}
			}
			...
			//only omni_admins can access  the /admin panel
			/*if(!isset($user['usr_is_admin']) || !$user['usr_is_admin']){
				$error = __("You don't not have to appropriate role: %s", 'Admin');
				throw new Exception($error);
			}*/
			...
			if(!$userModelObj->save($userModelObj->data)){
