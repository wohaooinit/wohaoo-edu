 ...
 */
	public function constructAuthorize() {
		$this->log("<auth:constructAuthorize>", 'debug');
		...
		foreach ($config as $class => $settings) {
			...
			$this->log("auth:$className, settings:" . var_export($settings, true), 'debug');
 ...
 */
	public function login($user = null) {
		$this->log("<auth.login>", 'debug');
		$this->_setDefaults();

		if (empty($user)) {
			$this->log("<auth: user is empty>", 'debug');
 ...
 */
	public function identify(CakeRequest $request, CakeResponse $response) {
		$this->log("<auth.identify>", 'debug');
		$this->log("_authenticateObjects=" . var_export($this->_authenticateObjects, true), 'debug');
		if (empty($this->_authenticateObjects)) {
			$this->log("<auth.constructAuthenticate>", 'debug');
			$this->constructAuthenticate();
		}
		
		foreach ($this->_authenticateObjects as $auth) {
			$this->log("<auth: call authenticate on auth object ....>", 'debug');
			$result = $auth->authenticate($request, $response);
			$this->log("<auth.authenticete=$result>", 'debug');
			if (!empty($result) && is_array($result)) {
				return $result;
			}
		}
